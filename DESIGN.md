...
- **The note reads at 16px across window widths and in print,** at the default root size. The preset uses `--typeset-size: 1rem` and sets `font-size` from that token without the vendored multiplier. Resizing the window changes wrapping without changing the text size.
- **Code size is independent of prose size.** Fenced code and source mode use `0.875rem` and `1.5` line-height. Inline code uses `0.9em` to follow the sentence around it. The three faces remain unchanged; the comparison controls belong to the prototype, not the app (`D73`).
...
- **In chrome the mono is for identity and machine text.** The wordmark and a suggestion's shorthand hint. Everything else in the interface is sans. Inside the note it does the job the table above gives it, which is code.
...
- **The note and source editors use shadcn's ScrollArea** (`D76`). Its viewport owns scrolling and its scrollbar supplies the appearance. The scrollbar appears during scrolling and stays visible while held, then hides when idle. Hover alone does not reveal it. Focus mode hides that scrollbar. Other native scroll surfaces retain system colors, width, and visibility behavior. Forced-colors mode uses system selection colors.
...
- Shadcn primitives carry their own hover and press transitions, and the toast spinner spins. Beyond those and the tab strip, `src/styles.css` animates focus mode, the editor scrollbar, wikilinks, and the code-block toolbar and its buttons.
...
- **The titlebar carries the note's identity** (`D28`): what it is called, whether it is pinned, and whether it is written (`D38`). Those sit in the drag region rather than in a band of their own, so the active tab's label is the note title, resolved from a leading heading, the first readable body line, imported frontmatter, then the filename, as specified in `SPEC.md` (`D52`). It is display-only text: renaming a file is a ⌘⇧P action. `Titlebar` in `src/components/titlebar.tsx` declares the region both windows render, as `deep`: a press anywhere in the band moves the window unless it lands on a control, and Tauri treats a button, an input, or a `tab` role as that boundary, so a control needs no class to stay clickable (`D80`). `src/components/tabs/tab-strip.tsx` is the one place with more to say. A tab among neighbours marks its box `false` and leaves its label button unmarked, so a press on its padding moves nothing and the label button stays the only drag handle, and a lone tab puts the region on its label button instead, so pressing and moving it moves the window rather than the tab (`D60`). With no tab open the strip is its `+` alone and the press reaches the band.
- **The save state is a glyph, and it sits in the titlebar** (`D35`, `D38`). `SaveIndicator` renders after the tab strip and before the pin (`D52`), a floppy carrying a pen for `dirty`, no badge for `saving`, a check for `saved`, and a slash for `failed`, at `CHROME_GLYPH` in the same 24px box the pin and the view toggles use. `saved` takes the pin's idle `opacity-60` and `failed` takes `--destructive`; `dirty` and `saving` inherit the bar's, so the glyph is lit while a write is outstanding and dim once it lands. The word reaches a hover tooltip and `sr-only` text, never the bar. It follows the active tab, so the empty state's titlebar holds the strip's `+` alone. It is not a control, so a press on it moves the window like the band around it, and hover still opens the tooltip (`D80`).
...
- **The titlebar holds the open tabs, then the note's controls** (`D52`). Both bands are `--card`, so the note's `--background` reads as its own surface. A tab is a shaped item at the chrome rung, 24px tall and `rounded-md`, the same box the `+` and the overflow count use. Tabs sit in a `gap-0.5` run and space separates them, with no divider. The active one carries `--muted` and `--foreground`, and an inactive one carries `--muted-foreground` with `--muted` at 40% on hover, so a hovered tab never reads stronger than the active one. Tabs divide the strip evenly between `min-w-24` and `max-w-56` and scroll past that, so a lone tab is one capped item with drag region beside it. External files use the same readable titles and sans typography as library notes. A dragged tab takes a shadow and paints over the tabs it crosses (`D60`). The save glyph and the pin sit after the strip and follow the active tab.
...
- **The review** replaces the editor pane the way graph view does, in the same `max-w-2xl` column with the editor's padding, and nothing animates the swap. It lists the places that changed on both sides in order. The quoted text sits inside the `.typeset-note` preset, so context, sides and the result box take the note's face, size and leading from the same rules as the editor, and a place whose lines are one heading declares `data-heading="1"` and takes the h1 scale from the same rule as an h1. Paragraphs that combined on their own sit between them as muted context, and a run past six lines folds behind a link `Button` reading "n unchanged lines". A place shows "on disk" and "mine" as read-only blocks on the card surface, each with a ghost "use this" whose accessible name starts with the visible label and names the side, over one "result" `Textarea` at the sides' radius, empty until a side is used or the user types, with the placeholder "use one side above, or write the result". A side with nothing reads "nothing" in the muted ink. The result box is labelled by a `Label`, and when typing has removed every overlap the header is an `Empty` reading "your edits no longer overlap the change on disk" over "resolve applies both". The footer holds "resolve", a ghost "back", the reason resolve waits in tabular figures ("2 of 2 still need a result", then "every place has a result"; a used empty side counts), and the two chords drawn by `Chord` inside `KbdGroup`s. Opening puts focus on the first result; Esc goes back and returns focus to "review"; ⌘⏎ resolves and returns focus to the note. No toast follows, since the glyph reading saved is the confirmation every save gets.
...
Palette filter choices open from an "add filter" button below the find results. They use their own input and return to the saved note query. Escape goes back one view before dismissing the palette. Picking a filter preserves the rest of the query. Pending searches retain the last displayed note rows without dimming them. Retained rows are disabled until the current query completes, so keyboard and pointer activation cannot open an earlier result. Loading does not replace the list with a message; only a first read against a scanning index, with no rows to retain, shows `indexing notes...` where the rows will land. A Shadcn spinner appears after a read takes 500ms, in reserved space at the right of the footer. Debounce produces no loading feedback. Result rows and filter choices occupy separate lists, so a pending search cannot select a filter. List views share a 24rem palette height capped to the window, leaving the input and footer outside the scrolling list. Rename and delete remain compact. Result changes do not animate the height or fade the rows. A changed query selects its first completed result and returns the list to the top; a refresh of the same query preserves the chosen note.
