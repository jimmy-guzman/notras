...
- UI text sits at `text-xs` for secondary information (status strip, palette metadata, tag chips) and inherits the base size otherwise. Do not invent a per-component size. The sizes in hand-written CSS for floating surfaces, code-block chrome, and source mode are not precedents.
- **In chrome the mono is for identity and machine text.** The wordmark and a suggestion's shorthand hint. Everything else in the interface is sans.
...
- **Every colour is a hex CSS variable in `src/styles.css`.**
- **Dark is the default.** `:root` defines the dark palette and `@media (prefers-color-scheme: light), print` redefines the same variables, so paper is light whatever the system is set to. There is no theme toggle; the system decides.
- **`--primary` is the accent. `--accent` is not.** shadcn uses `--accent` for hover surfaces.
...
- **Syntax highlighting uses twelve named roles,** `--syntax-comment` through `--syntax-type`. Shiki maps TextMate scopes to those CSS variables. Declarations use pink, control flow orange, imports violet, functions and members blue, and types cyan. Code stays at regular weight with no syntax-driven italics, including comments and Markdown source emphasis (`D73`).
- **Two neutral text tones and an error ink.** `--foreground` carries content, `--muted-foreground` carries secondary information, and `--destructive` carries failures. `--faint`, for next-step lines and placeholders, shares secondary ink.
...
- **The note and source editors use shadcn's ScrollArea** (`D76`). Its viewport owns scrolling and its scrollbar supplies the appearance. Focus mode hides it.
...
The recipe derives sRGB hex values before they enter the stylesheet. It uses [Oklab's linear-sRGB conversion matrices](https://bottosson.github.io/posts/oklab/) and [WCAG relative-luminance contrast](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum.html). No color computation runs in the app.

1. Neutrals share OKLCH hue `70` and chroma `0.008`.
...
5. Dark secondary lightness is `0.675`, since `0.670` measured 4.47:1 on the hover surface.
...
Body ink measures about 10.24:1 on light paper and 8.74:1 on dark paper. Colored syntax inks measure at least 4.77:1 in light and 6.77:1 in dark on paper and card. The modes share hue assignments, not identical contrast ratios.
...
- **The desktop tile retains the grid and finish of `D34`.** The mark occupies 136 of the tile's 192 units. The script draws the tile once rather than maintaining a second shape in SVG.
- **The tray is a monochrome template.** Its export bypasses the size ladder, passing an 8-unit seam to `write_mark` and rasterizing one 36px asset (`D33`). The separating seam and folded corner are transparent so the system can tint the sheets for either menu-bar appearance.
...
- **The titlebar carries the note's identity** (`D28`): what it is called, whether it is pinned, and whether it is written (`D38`). Those sit in the drag region rather than in a band of their own, so the active tab's label is the note title, resolved as `SPEC.md` specifies (`D52`). It is display-only text: renaming a file is a ⌘⇧P action.
...
- **The save state is a glyph, and it sits in the titlebar** (`D35`, `D38`). `SaveIndicator` renders after the tab strip and before the pin (`D52`), a floppy carrying a pen for `dirty`, no badge for `saving`, a check for `saved`, and a slash for `failed`, at `CHROME_GLYPH` in the same 24px box the pin and the view toggles use. `dirty` and `saving` take `--foreground`, `failed` takes `--destructive`, and `saved` inherits the bar's `--muted-foreground`, so the glyph is lit while a write is outstanding and dim once it lands. The word reaches a hover tooltip and `sr-only` text, never the bar. It follows the active tab. It is not a control, so a press on it moves the window like the band around it, and hover still opens the tooltip (`D80`).
- **Anything in chrome that turns on and off is a `Toggle`** (`D37`), at `size="icon-xs"`, showing its on-state as `--primary`. That covers the four view toggles, which are one `ToggleGroup` and so one tab stop with arrow keys inside it, and the pin in the titlebar. Idle is the `--muted-foreground` both bands set. Hover is a surface and pressed is the accent ink, which is what keeps the two states apart; `CHROME_TOGGLE` in `src/lib/ui/chrome.ts` carries that override and the ghost button's dark hover, which `Toggle` ships without.
...
- **Tags are picked from the status strip** (`D30`). A chip reads `#groceries` and filters to that tag; the `TagPlus` control before the chips opens the combobox, whose vocabulary is the index's own counted tag list. It is built as the new-note button is: an icon-only ghost `Button` named `edit tags` by its tooltip, since the picker removes as well as adds. It leads the run rather than following it, so it stays put as tags come and go and reads as the label of what follows; with none it stands alone. The chips are the only part that shrinks, so they clip before the button or the word count does.
...
- **Two bands frame the note, and there is no third** (`D52`). The titlebar above and the status strip below have no dividing borders or inset shadows. Quick capture also has no dividing edge on its titlebar or bottom hint strip. Both windows place the note on an inset `--background` surface within the `--card` shell. The frame has 4px side margins, an 8px outer radius, and a fixed 4px inner rim; each scroller clips its own content inside the rim, and the scrollbar crosses the rim to ride the frame's edge. No extra vertical margin separates the frame from the bands. With no status strip, the empty workspace leaves 4px below the frame. The rim keeps code blocks separate from the chrome while scrolling.
- **The titlebar holds the open tabs, then the note's controls** (`D52`). Both bands are `--card`, so the note's `--background` reads as its own surface. The save glyph and the pin sit after the strip and follow the active tab.
- **A tab is a shaped item at the chrome rung**: 24px tall and `rounded-sm`, the box the new-note button and the overflow count use. Tabs sit in a `gap-1` run with no divider. The active tab carries the workspace's `--background` and `--foreground`; an inactive tab is transparent in the band's `--muted-foreground`. Tabs divide the strip evenly between `min-w-24` and `max-w-56` and scroll past that, so a lone tab is one capped item. External files use the same readable titles and sans typography as library notes. A dragged tab takes a shadow and paints over the tabs it crosses (`D60`).
- **Every control in both bands uses the ghost-button hover treatment**: `--muted` in light mode, `--muted` at 50% in dark mode, and `--foreground` text. The new-note button and the overflow count are ghost `Button`s at `icon-xs` and `xs`.
- **The palette is the action surface.** A new action on the note, a tab, or the app goes in ⌘⇧P rather than into new chrome. `SPEC.md` lists what it holds. ⌘P finds a note and ⌘⇧P acts on the app, on the tab that is showing, or on the note inside it, according to what the action reads.
...
- **A chrome control's tooltip names its action and carries its chords.** For an icon-only control the name is its accessible name: `new note`, `pin note`, `edit tags`, `focus mode`. For a chip whose label is a fact, the tooltip names the action behind it: `3 mentions` opens `show mentions`. A binding registered in another file is found through `useChordsByName` under that name, so the tooltip and the palette row read one registration and a control with no binding shows the name alone; one registered beside the tooltip shares its constant instead. A tab and a tag chip are their own label and carry none.
...
- **A note that changed on disk under overlapping edits** shows the same destructive `Alert` in the same place: "this note changed on disk" over "Your unsaved edits overlap the change, so nothing saves until you review them", with one outline button in its action slot, "review".
...
- **The review footer** holds "resolve", a ghost "back", the reason resolve waits in tabular figures ("2 of 2 still need a result", then "Every place has a result"; a used empty side counts), and the two chords drawn by `Chord` inside `KbdGroup`s. Opening puts focus on the first result; Esc goes back and returns focus to "review"; ⌘⏎ resolves and returns focus to the note. No toast follows, since the glyph reading saved is the confirmation every save gets.
