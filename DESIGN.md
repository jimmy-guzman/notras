...
- **Scrollbars use the platform's colors and behavior.** `scrollbar-color: auto` and `color-scheme` let the webview choose colors for the current scheme. Width, dragging, and visibility timing follow the platform, including its "always show" preference. Focus mode still hides its scrollbar. Forced-colors mode uses system scrollbar and selection colors.
