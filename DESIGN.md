...
- **The titlebar carries the note's identity** (`D28`): what it is called and whether it is pinned. Those sit in the drag region rather than in a band of their own, so the active tab's label is the note title, resolved as `SPEC.md` specifies (`D52`). It is display-only text: renaming a file is a ⌘⇧P action.
...
- **The titlebar holds the open tabs, then the pin** (`D52`). Both bands are `--card`, so the note's `--background` reads as its own surface. The pin sits after the strip and follows the active note; an external file has none.
