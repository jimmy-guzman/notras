...
- **Two bands frame the note, and there is no third** (`D52`). The titlebar above and the status strip below have no dividing borders or inset shadows. Quick capture also has no dividing edge on its titlebar or bottom hint strip. Both windows place the note on an inset `--background` surface within the `--card` shell. The frame has 4px side margins, an 8px outer radius, and a fixed 4px inner rim around a clipped surface with a 4px radius. No extra vertical margin separates the frame from the bands. With no status strip, the empty workspace leaves 4px below the frame. The rim keeps code blocks separate from the chrome while scrolling.
