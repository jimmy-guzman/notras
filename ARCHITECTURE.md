...
- **Library IO binds to the directory handle it validated.** `RelativePath` rejects empty, hidden and nonportable components and serializes paths with `/` separators. `locate` walks the folder through no-follow directory opens and refuses a symlinked component, and every read, write, move, deletion and attachment write then runs on that handle, so a component replaced after validation cannot redirect it. A validated folder that is moved during an operation carries the result with it; a later scan removes rows for what is no longer reachable. Scans skip symlinked entries. In-place writes to an open file remain unisolated. Explicit external-file reads and writes use user-selected host paths, opened through the file's parent directory without following a symlink at the file itself. Attachment imports may read a selected external source, but their destination uses the library boundary.
