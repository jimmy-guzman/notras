...
Palette filter choices open from an "add filter" button below the find results. They use their own input and return to the saved note query. Escape goes back one view before dismissing the palette. Picking a filter preserves the rest of the query. Pending searches hide previous results so Enter cannot open a note from an earlier query. Result rows and filter choices occupy separate lists, so a pending search cannot select a filter. The palette caps its height to the window, leaving the input and footer outside the scrolling list.
