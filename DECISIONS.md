...
Scrollbars retain the platform's colors and behavior. The owner found both full-strength secondary ink and an 80% ink blend too prominent, and reported that the tinted scrollbar stayed visible while idle. Rejected: custom scrollbar colors and weakening the shared text token, whose hover contrast is already close to the text minimum. Scrollbar visibility follows the platform, including its "always show" preference. Rejected: app-controlled hiding that overrides that preference. Selection in editors and inputs uses the palette. Completed tasks retain secondary ink and strikethrough without another opacity reduction. Native controls keep their system appearance. Rejected: replacing scrollbar geometry or native popups to force identical rendering across webviews.
...
### D76 The editor owns a shadcn scroll area

The owner requested shadcn's scrollbar after reporting two scrollbars in the installed app. The note and source editors use the generated ScrollArea component. Focus mode scrolls its viewport and hides its scrollbar. This replaces the editor portion of `D73`'s native-scrollbar choice.

The document and body have viewport height and hidden overflow. Overscroll containment alone leaves document scrolling available when content overflows. Rejected: replacing the editor's scrollbar without constraining the document, because appearance does not remove the second scroll container. Rejected: hiding the outer scrollbar while allowing the window to scroll, because the two bands must stay stationary.

Tauri adds a nonce to the launch stylesheet and the packaged style policy. Base UI and Tiptap receive that nonce for the styles they inject at runtime. Without it, the packaged policy rejects Base UI's native-scrollbar hiding rule, leaving native bars on both axes beside shadcn's scrollbar. Rejected: copying that rule into the app stylesheet or disabling Tauri's policy modification, because the former leaves other runtime styles blocked and the latter changes the policy to accommodate a missing nonce.

The owner requested that the scrollbar appear only during scrolling and approved Base UI's built-in scrolling state, including keyboard scrolling. CSS controls opacity and pointer interaction from that state, with `:active` keeping a held thumb visible through a pause. Rejected: separate scroll listeners and timers, because Base UI already supplies the state and its idle timeout. The owner found the initial fade too abrupt, so the hide transition lasts longer than the reveal. The generated component and scroll containers retain their existing behavior.

The stylesheet registers Base UI's four overflow-distance properties with `<length>` syntax, a `0px` initial value, and inheritance disabled. Base UI skips this optimization in WebKit because explicit inheritance of registered properties is unreliable there. The editors use no inherited overflow effects. Without registration, each scroll updates inherited properties on the viewport, then a scroll-position read forces style recalculation through the note's descendants. A sample of the running development app located that work in WebKit's scroll-event path. Repeated comparisons in a separate WebKit window using the production web bundle and the owner's syntax-review note measured mean scroll handling near 49 ms with inheritance and 0.4 ms without it. Removing the registrations restored the slowdown. Rejected: disabling the fade or resetting the properties on the viewport's children, because neither reduced the measured cost. These measurements isolate the cause; installed-app scrolling remains the release check.

### D77 The index leaves the library, and library IO binds to directory handles

`RelativePath::resolve` checked each path component for a symlink and then returned a `PathBuf`, and every library operation did pathname IO on it. CodeRabbit and a local probe confirmed the gap in #178: rename the validated folder away, plant a symlink at its old name, and the write lands outside the library. A pathname check cannot close that race, because the check and the operation resolve the path separately.

Library IO now opens the resolved root as a `cap-std` directory handle, walks folders with no-follow opens, and runs every read, write, move, deletion and attachment write on the handle it validated. Temporary siblings are created and published inside that handle, with `renameat2`/`renameatx_np` no-replace flags on Linux and macOS and a rename by file handle on Windows. The index moves to the app cache directory, keyed by a SHA-256 of the resolved root, because SQLite opens its database by pathname and exposes no descriptor to bind; a database outside the library is out of reach of anything that can plant a symlink inside it. The defect was the index's location, not its engine.

**Rejected: replacing SQLite.** No cost in #180's measurements is one SQLite fails to meet, and an in-memory index would lose the WAL read snapshots the read-view design depends on.

**Rejected: keeping `.notras/index.db` with a symlink preflight.** The preflight is the same check-then-use pattern the rest of the change removes, so the race stays open for the one file that holds every derived row.
...
**Constraint:** an operation follows its validated folder if that folder moves during the operation, inside or outside the library. The next scan removes rows for anything no longer reachable. Older `.notras/` folders inside a library are left in place; deleting a user's files without asking is the wrong default for a cache move.
