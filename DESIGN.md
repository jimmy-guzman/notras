...
- **The review** replaces the editor pane the way graph view does, in the same `max-w-2xl` column with the editor's padding, and nothing animates the swap. It lists the places that changed on both sides in order. Paragraphs that combined on their own sit between them as muted context in the note face, and a run past six lines folds behind a text button reading "n unchanged lines". A place shows "on disk" and "mine" as read-only blocks on the card surface, each with a ghost "use this" whose accessible name says which side, over one "result" `Textarea` in the note face, empty until a side is used or the user types, with the placeholder "use one side above, or write the result". A side with nothing reads "nothing" in the muted ink. A place whose lines are one heading takes the note's h1 size. A run past six lines folds behind a link `Button`, the result box is labelled by a `Label`, and when typing has removed every overlap the header is an `Empty` reading "your edits no longer overlap the change on disk" over "resolve applies both". The footer holds "resolve", a ghost "back", the reason resolve waits ("2 of 2 still need a result", then "every place has a result"; a used empty side counts), and the two chords drawn by `Chord` inside `KbdGroup`s. Opening puts focus on the first result; Esc goes back and returns focus to "review"; ⌘⏎ resolves and returns focus to the note. No toast follows, since the glyph reading saved is the confirmation every save gets.
