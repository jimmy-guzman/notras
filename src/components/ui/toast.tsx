...
function Toast({ className, ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(
        "group/toast text-popover-foreground focus-visible:border-ring focus-visible:ring-ring/50 pointer-events-auto absolute right-0 bottom-0 z-[calc(1000-var(--toast-index))] w-full origin-bottom rounded-2xl border bg-[color-mix(in_oklab,var(--toast-ink,var(--popover))_7%,var(--popover))] shadow-lg will-change-transform outline-none select-none focus-visible:ring-[3px] data-[type=error]:[--toast-ink:var(--destructive)] data-[type=info]:[--toast-ink:var(--info)] data-[type=success]:[--toast-ink:var(--success)] data-[type=warning]:[--toast-ink:var(--warning)]",
...
function ToastTitle({ className, ...props }: ToastPrimitive.Title.Props) {
  return (
    <ToastPrimitive.Title
      ...
      className={cn(
        "text-sm font-medium text-(--toast-ink,--foreground)",
        className
      )}
      {...props}
    />
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  return (
    <span
      ...
      className="shrink-0 text-(--toast-ink,--foreground) [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4"
    >
      {icon}
    </span>
  );
