...
- An empty new note is `untitled.md` in the notes root. Creating a note with content and no explicit name derives its filename from its resolved title. A name already taken takes the next free `untitled-2`, then `untitled-3`. The suffixed name stays within 120 characters, with the base cut to make room.
