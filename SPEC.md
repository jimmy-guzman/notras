...
- A link activation waits for the note list when resolution is not yet available. A failed read reports "could not open note" and its reason without claiming the destination is missing. A newer link activation, a change of tabs, or closing the originating session supersedes the pending action. Its eventual result cannot navigate or show an obsolete failure.
