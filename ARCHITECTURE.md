...
```sql
note(id INTEGER PK, path TEXT NOT NULL UNIQUE, title TEXT, folder TEXT, pinned INT, created_at INT, updated_at INT)
...
`note.id` is an internal SQLite key, shared with `note_fts.rowid`. It remains stable through note updates and database compaction. Paths remain note identity outside the index. Updates and deletes address FTS rows through this key; deleting a note removes its FTS entry before its key disappears, within the same transaction. Schema version 7 rebuilds older indexes whose FTS row IDs were allocated independently.
