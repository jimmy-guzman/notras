...
- **Actions:** oRPC (`@orpc/server`) for type-safe `.actionable()` server actions; `@orpc/react` / `@orpc/react/hooks` for client-side `useServerAction` and `createFormAction`
...
src/
  ...
  lib/              # Client utilities, search params, orpc builder
...
- **Server actions** live in `src/actions/`. Three patterns coexist:
  - **oRPC `.actionable()` pattern** (mutations): Import `authedProcedure` from `@/lib/orpc`. Define as `authedProcedure.input(Schema.standardSchemaV1(schema)).handler(async ({ context, input }) => { ... })` then export the result of `.actionable()`. The `authedProcedure` middleware resolves `userId` via `AppRuntime` and injects it into `context`. Validation is automatic. Result type is a `[error, data]` tuple — clients destructure `const [error, result] = await someAction(input)` and check `if (error)`. Use `useServerAction` from `@orpc/react/hooks` for client-side state (`action.isPending`, `action.isSuccess`); `onSuccessDeferred` / `onErrorDeferred` interceptors from `@orpc/react` for deferred side-effects (dialog close, navigation, error feedback). Prefer `onErrorDeferred` for error toasts; only use `onSuccessDeferred` for a success toast when the outcome isn't self-evident from the UI (e.g. a setting saved without a visible state change) — if the UI already reflects the change (item appears/disappears, redirect fires), skip the success toast. Throw `ORPCError` from `@orpc/server` (not plain `Error`) for expected failures so the error message is reliably propagated to `error.message` on the client.
  - **oRPC `createFormAction` pattern** (FormData mutations that redirect): For forms submitted via native `<form action={...}>` (i.e., `FormHotkeys`) that need a server-side `redirect()` after success. Define an `authedProcedure` chain as above but **do not** call `.actionable()` — instead wrap it with `createFormAction` from `@orpc/react`: `export const myAction = createFormAction(myProcedure)`. The procedure receives deserialized bracket-notation FormData as typed `input`, calls `redirect()` directly inside the handler (not in an interceptor — `redirect` must throw synchronously during the request). Use `Schema.optionalWith(schema, { default: () => defaultValue })` for fields that may be absent from the FormData (e.g., a `tags` field not present on a create form). The file still needs `"use server"` at the top. `createFormAction` returns `(form: FormData) => Promise<void>`; `FormHotkeys` accepts `(formData: FormData) => Promise<void> | void`, so `Promise<void>` is a valid subtype and `myAction` can be passed directly as `FormHotkeys`'s `action` prop.
  - **Read actions** (no `"use server"`, called directly from RSCs): Files like `get-notes.ts`, `get-note.ts`, `get-links.ts` export plain `async function`s with no directive. They call `AppRuntime.runPromise` to resolve the `userId` and fetch data, and use `"use cache"` at the top of cacheable function bodies with `cacheTag(...)` as the first statement. For queries that depend on relative time filters (e.g. `time: "today" | "yesterday" | ...`), caching is bypassed entirely — a separate uncached helper is called instead, since cached results would be stale immediately. Read actions are never wrapped in oRPC or called from client components.
...
- **Validation** uses Effect Schema from `src/server/schemas/`. For oRPC `.actionable()` server actions, validation is automatic via `authedProcedure.input(Schema.standardSchemaV1(schema))`.
...
- **`useServerAction` pattern:** For forms that use oRPC `.actionable()` server actions, use `useForm` from `react-hook-form` + `useServerAction` from `@orpc/react/hooks`. Bridge validation via `standardSchemaResolver` from `@hookform/resolvers/standard-schema` with `Schema.standardSchemaV1(schema)`. Submit via `form.handleSubmit(async (data) => { await action.execute(data) })`. For loading state, check `action.isPending`. Use `onSuccessDeferred` / `onErrorDeferred` interceptors from `@orpc/react` for deferred side-effects — no `useEffect` needed — preferring `onErrorDeferred` for error toasts and reserving `onSuccessDeferred` for dialog close, navigation, or a success toast only when the outcome isn't obvious from the UI. For reactive field values, prefer `useWatch` over `form.watch()` (React Compiler compatibility). Use `Controller` for non-native inputs (file inputs needing `File` extraction, Radix Select) and `register()` for simple native text/email inputs. For forms that redirect after submit (e.g., create/edit note), use `createFormAction` instead.
