...
- Closing the last tab leaves the empty state, where ⌘E, ⌘⌥G, ⌘⌥⇧W and ⌘⇧Y do nothing, the status strip is hidden, and the title bar holds the strip's new-note button alone. ⌘D still sets the writing mode, and the next note opened is already in it. Pressing and moving the bar beside it moves the window, and a double-click zooms.
...
- A pressed view toggle and the pin of a pinned note use a neutral muted fill and body-ink glyph. The fill remains while the control is pressed, including when the pointer leaves it.
- The tag picker, the mentions list, and every tooltip open 4px off their bar. Hovering the new-note button, the pin, a view toggle, the tag control, or the mentions count shows a tooltip naming the action, with every shortcut bound to it beside the name: `new note` ⌘n ⌘t, `pin note` or `unpin note`, `focus mode` ⌘d, `markdown source` ⌘e, `graph view` ⌘⌥g, `edit tags` ⌘⇧y, `show mentions` ⌘⇧l.
- Both windows use 32px chrome bands. The 24px tab and control rows have 4px above and below them. Tabs, tag chips, the mentions and overflow counts, and the word count inset their text 6px. Neighboring tabs, titlebar controls, tag chips, and view toggles have 4px gaps; flexible space still separates the left and right groups. Native window controls retain their reserved clearance.
- The note surface has 4px side insets, 8px outer corners, and a fixed 4px inner rim in both windows. Scrolling content stays clipped inside the rim, separating code blocks from the chrome. The editor scrollbar crosses the rim and rides the frame's edge, its thumb ending 1px inside it. No extra vertical margin separates the frame from the bands. The empty workspace, which has no status strip, leaves 4px below its frame.
