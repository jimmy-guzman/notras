...
session lives through the rename instead of remounting (`D56`). Finding the tab
that holds a path is a lookup in `store.ts`, never a key someone rebuilds.
Several are alive at once and only the active one is shown, which is what makes
a switch lossless (`D53`). Autosave in
