...
- **The palette is the action surface.** A new action goes in ⌘⇧P rather than into new chrome. `ARCHITECTURE.md` lists what it holds. ⌘P finds a note and ⌘⇧P acts on the tab that is showing, and on the note inside it where the action reads one. Neither lists the other's rows.
