...
- Copying or saving a code block writes a backtick fence of at least three characters, longer than any backtick run in its content. Nested fence examples retain their literal text and language on reopening, and the prose and code following them retain their structure.
...
- Highlighting loads from packaged assets and works offline. A grammar may appear after the text, without moving the caret, changing note content, or adding an undo step. A loading failure leaves editing available and reports "could not highlight code"; reopening the editor retries loading.
