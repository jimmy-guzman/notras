...
describe(layoutRing, () => {
  it("should sit a lone neighbour on each side level with the centre", () => {
    ...
    expect(ring.incoming).toStrictEqual([{ angle: 180, x: 0.12, y: 0.5 }]);
    expect(ring.outgoing).toStrictEqual([{ angle: 0, x: 0.88, y: 0.5 }]);
  });

  it("should spread neighbours top to bottom on both sides", () => {
    const ring = layoutRing(3, 3);

    expect(ring.incoming.map((node) => node.y)).toStrictEqual(
      ring.incoming.map((node) => node.y).toSorted((a, b) => a - b)
    );
    expect(ring.outgoing.map((node) => node.y)).toStrictEqual(
      ring.outgoing.map((node) => node.y).toSorted((a, b) => a - b)
    );
    expect(ring.incoming.every((node) => node.x < 0.5)).toBeTruthy();
    expect(ring.outgoing.every((node) => node.x > 0.5)).toBeTruthy();
  });
  ...
  it("should give nothing for an empty side", () => {
    expect(layoutRing(0, 0)).toStrictEqual({
      incoming: [],
      outgoing: [],
      top: [],
    });
  ...
  it("should lay what holds the note along the top, left to right, and narrow the sides for it", () => {
    ...
    expect(ring.top.every((node) => node.y < 0.5)).toBeTruthy();
...
describe(layoutRound, () => {
  it("should space members evenly around the ring from the top, clockwise", () => {
    expect(layoutRound(4).map((node) => node.angle)).toStrictEqual([
      -90, 0, 90, 180,
    ]);
    expect(layoutRound(1)[0]?.y ?? 1).toBeLessThan(0.5);
    expect(layoutRound(0)).toStrictEqual([]);
  });
});
