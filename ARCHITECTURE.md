...
- **A save carries the revision it started from.** `save_note` and `write_external` take `expected` and answer with a `SaveOutcome`; the engine writes nothing when the file's hash differs, so no path exists for a session to overwrite a version it has not seen.
