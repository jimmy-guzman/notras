...
- **Server actions** live in `src/actions/`. Three patterns coexist:
  - **oRPC `.actionable()` pattern** (mutations): Import `authedProcedure` from `@/lib/orpc`. Define as `authedProcedure.input(Schema.standardSchemaV1(schema)).handler(async ({ context, input }) => { ... })` then export the result of `.actionable()`. The `authedProcedure` middleware resolves `userId` via `AppRuntime` and injects it into `context`. Validation is automatic. Result type is a `[error, data]` tuple — clients destructure `const [error, result] = await someAction(input)` and check `if (error)`. Use `useServerAction` from `@orpc/react/hooks` for client-side state (`action.isPending`, `action.isSuccess`); `onSuccessDeferred` / `onErrorDeferred` interceptors from `@orpc/react` for deferred side-effects (toasts). Throw `ORPCError` from `@orpc/server` (not plain `Error`) for expected failures so the error message is reliably propagated to `error.message` on the client.
