...
- **Server actions** live in `src/actions/`. Two patterns coexist:
  ...
  - **Plain `"use server"` async functions** (FormData mutations only): `createNote` and `updateNote` are plain `async function` with `"use server"` — they receive `FormData` and are passed directly as `action={fn}` to `<form>`. No oRPC wrapper; validation is manual via `Schema.decodeUnknownSync`.
