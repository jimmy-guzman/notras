...
- **Source mode** strips that chrome: one bare, transparent, wrapping code block inheriting the fenced-code `1rem` size and `1.7` line-height, so ⌘E reads as the same document rather than a different screen.
