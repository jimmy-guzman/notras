...
Library IO now opens the resolved root as a `cap-std` directory handle, walks folders with no-follow opens, and runs every read, write, move, deletion and attachment write on the handle it validated. Temporary siblings are created and published inside that handle, with `renameat2`/`renameatx_np` no-replace flags on Linux and macOS and `NtSetInformationFile` with a root directory handle on Windows, since the Win32 wrapper rejects one. The index moves to the app cache directory, keyed by a SHA-256 of the resolved root, because SQLite opens its database by pathname and exposes no descriptor to bind; a database outside the library is out of reach of anything that can plant a symlink inside it. The defect was the index's location, not its engine.
